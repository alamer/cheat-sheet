...
begin
    p_table_name:='***';
    for i in ( SELECT PARTITION_NAME, HIGH_VALUE FROM user_tab_partitions WHERE TABLE_NAME = upper( p_table_name ) and partition_name not like 'P_OPER_HIS_FIRST%' ORDER BY PARTITION_POSITION)
    loop    
        execute immediate 'select ' || i.HIGH_VALUE || ' from dual' into l_partition_high_value_dt;
        if (l_partition_high_value_dt<sysdate-5) then
